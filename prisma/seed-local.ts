...
async function main() {
  ...
  // Create sample users (parents) - SAME IDs as production for consistency
  const parent1 = await prisma.user.upsert({
    where: { email: 'parent@test.com' },
    update: {},
    create: {
      id: 'cmfc5uege0000xt2sxukou174', // Same ID as production
  ...
  const parent2 = await prisma.user.upsert({
    ...
    update: {},
    create: {
      id: 'cmfc5uegf0002xt2s8u9ox175', // Same ID as production
      email: 'demo@test.com',
      name: 'Demo Parent',
  ...
  console.log('👤 Created users:', { parent1: parent1.id, parent2: parent2.id })
